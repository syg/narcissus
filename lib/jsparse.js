...
Narcissus.parser = (function() {
        ...
        switch (tt) {
          ...
          case CONTINUE:
            ...
            if (label) {
                ...
                // non-label statement.
                while (i < ss.length - 1 && ss[i+1].type === LABELED)
                    i++;
          ...
          default:
            if (tt === IDENTIFIER) {
                ...
                if (tt === COLON) {
                    ...
                    t.get();
                    n = new Node(t, { type: LABELED, label: label });
    ...
    /*
     * Label :: (tokenizer, compiler context) -> label | null
     *
     * Parses an optional label attached to variable declarations, properties,
     * and lefthand side of assignments.
     */
    function Label(t, x) {
        var k = null;
        if (!t.match(LT))
            throw t.newSyntaxError("expecting label");
        var tt = t.get();
        if (tt !== IDENTIFIER)
            throw t.newSyntaxError("missing valid label");
        var label = t.token.value;
        if (label === 'HT')
            k = new definitions.Label(H, T);
        else if (label === 'HU')
            k = new definitions.Label(H, U);
        else if (label === 'LT')
            k = new definitions.Label(L, T);
        else if (label === 'LU')
            k = new definitions.Label(L, U);
        else
            throw t.newSyntaxError("label must be either " +
                                   "`HT', `HU', `LT', or `LU'");
        t.mustMatch(GT);
    ...
    function Expression(t, x) {
        ...
        // Expressions can be optionally labeled.
    ...
    function RelationalExpression(t, x) {
        ...
        while ((t.match(LT) || t.match(LE) || t.match(GE) || t.match(GT) ||
               ...
               t.match(INSTANCEOF))) {
            // Is this a label?
            if (t.token.type === LT && t.match(IDENTIFIER) && t.match(GT)) {
                t.unget(); t.unget(); t.unget();
                return n;
            }
    ...
    function UnaryExpression(t, x) {
        ...
        switch (tt = t.get(true)) {
          ...
          case DECLASSIFY:
            ...
            n.expression = AssignExpression(t, x);
            t.mustMatch(COMMA);
            n.infolabel = Label(t, x);
            t.mustMatch(RIGHT_PAREN);
            break;

          case LT:
            t.unget();
            n = new Node(t, { type: LABEL });
            n.infolabel = Label(t, x);
            n.expression = UnaryExpression(t, x);
            break;
