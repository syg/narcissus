...
Narcissus.interpreter = (function() {
    ...
    var narcissusGlobal = {
        ...
        Function: function Function(dummy) {
            ...
            // this function onto the fake compilation context.
            var f = parser.FunctionDefinition(t, null, false, parser.STATEMENT_FORM);
            var s = { ht: {}, hu: {}, lt: {}, lu: global,
                      base: mkBase(global), parent: null };
            return newFunction(f, { scope: s, pc: ExecutionContext.current.pc });
        },
        Array: function (dummy) {
            // Array when called as a function acts as a constructor.
            // XXX .length is broken in this partitioning for flow.
            var s = Array.apply(this, arguments);
            var base = mkBase(s);
            var pc = ExecutionContext.current.pc;
            var r;
            if (pc.c === H && pc.i === T)
                r = { ht: s, hu: [], lt: [], lu: [], base: base };
            else if (pc.c === H && pc.i === U)
                r = { ht: [], hu: s, lt: [], lu: [], base: base };
            else if (pc.c === L && pc.i === T)
                r = { ht: [], hu: [], lt: s, lu: [], base: base };
            else
                r = { ht: [], hu: [], lt: [], lu: s, base: base };

            definitions.defineProperty(r.base, 'length', s, false, false, true);
        ...
        print: function(v) {
            ...
            if (v instanceof Reference)
                narcissusGlobal.print(lookupReference(v));
            else if (v instanceof LabeledValue)
                print(pc + ":<" + v.k + ">" + getRawValue(v.r));
            else
                print(pc + ":" + getRawValue(v));
        }
    };
    ...
    // FIXME need push & friends to be aware of partitioning
    ...
    var globalScope = { ht: {}, hu: {}, lt: {}, lu: global,
                        base: mkBase(global), parent: null };
    ...
    ExecutionContext.prototype = {
        ...
        scope: globalScope,
        get scopeSpace() {
            return chooseBase(this.pc, this.scope);
        },
        thisObject: global,
        result: undefined,
        target: null,
    ...
    function FlowError(s) {
        this.msg = "flow violation: " + s;
    ...
    function LabeledValue(r, k, declassified) {
        this.r = r;
        this.k = k;
        this.declassified = declassified;
    }

    function Reference(store, propertyName, pc, node) {
        this.store = store;
        this.base = store.base[propertyName] || chooseBase(pc, store);
        this.propertyName = propertyName;
        this.storeLabel = chooseLabel(this.base, store, pc);
        this.node = node;
    }
    ...
    /*
     * Lifts functions to deal with either raw or labeled values.
     */
    function liftL(f) {
        return function() {
            for (let i in arguments) {
                if (arguments[i] instanceof LabeledValue)
                    arguments[i] = arguments[i].r;
            }
            return f.apply(this, arguments);
        }
    }

    function chooseLabel(b, s, pc) {
        if (!b)
            return pc;

        if (b === s.ht)
            return new Label(H, T);
        if (b === s.hu)
            return new Label(H, U);
        if (b === s.lt)
            return new Label(L, T);
        return new Label(L, U);
    }

    function chooseBase(k, s) {
        if (k.c === H && k.i === T)
            return s.ht;
        if (k.c === H && k.i === U)
            return s.hu;
    ...
    function getRawValue(v) {
        ...
        return r instanceof Reference ? getRawValue(lookupReference(r)) : r;
    }

    function getLabel(v, inRHS) {
        // Slow-path: label on v.
        if (v instanceof LabeledValue) {
            let r = v.r;
            // If the current value is a reference, join the labels.
            if (r instanceof Reference)
                return joinLabel(getLabel(r), v.k);
            return v.k;
        }
        ...
        if (v instanceof Reference) {
            let cell;
            if (v.base &&
                (cell = lookupReference(v)) instanceof LabeledValue) {
                // If the value has a label, use that.
                return cell.k;
            }
            // If not, use the implicit store label if we're assigning
            // into it.
            return inRHS ? v.storeLabel : null;
        }
        return null;
    }
    ...
    function putValue(v, w, vn) {
        ...
        if (r instanceof Reference) {
            let name = r.propertyName;
            if (!r.store.base[r.propertyName]) {
                r.base = chooseBase(r.storeLabel, r.store);
                r.store.base[name] = r.base;
            }
            return r.base[name] = w;
        }
    ...
    // rn is the node whose execute result was r.
    ...
    // Naive product lattice assumption: straightforward mapping to naturals.
    function valuateLabelC(k) {
        return definitions.latticeC[k.c];
    }

    function valuateLabelI(k) {
        return definitions.latticeI[k.i];
    }

    // For when we don't care to distinguish between HH and LL
    function valuateLabel(k) {
        return valuateLabelC(k) + valuateLabelI(k);
    }

    function joinLabel(k1, k2) {
        if (!k1 && !k2)
            return null;
        if (!k1) k1 = new Label(L, U);
        if (!k2) k2 = new Label(L, U);

        return new Label(valuateLabelC(k1) < valuateLabelC(k2) ? k2.c : k1.c,
                         valuateLabelI(k1) < valuateLabelI(k2) ? k2.i : k1.i);
    }
    ...
    function isDeclassified(u) {
        return u instanceof LabeledValue && u.declassified;
    }
    ...
    function execute(n, x) {
        ...
        var oldpc, slow, declassified = false;
        ...
        switch (n.type) {
          case FUNCTION:
            if (n.functionForm !== parser.DECLARED_FORM) {
                if (!n.name || n.functionForm === parser.STATEMENT_FORM) {
                    ...
                    let lt = new Object;
                    let lu = new Object;
                    x.scope = { ht: ht, hu: hu, lt: lt, lu: lu,
                                base: {}, parent: x.scope };
                    t = chooseBase(x.pc, x.scope);
          ...
          case IF:
            ...
            // XXX Since eval needs to get a hold of the pc counter as well,
            // we cannot unfortunately pass around pc functionally to execute
            // and have to do this explicit set/restore thing with x.pc.
            if (slow = isSlowPath(d, x)) {
                oldpc = x.pc;
                x.pc = joinLabel(x.pc, getLabel(d));
            }
          ...
          case WHILE:
            ...
            while (!n.condition || getRawValue(d)) {
                if (slow = isSlowPath(d, x)) {
                    oldpc = x.pc;
                    x.pc = joinLabel(x.pc, getLabel(d));
                }
          ...
          case FOR_IN:
            ...
            // Compute slow/fast once outside of loop.
            if (slow = isSlowPath(s, x)) {
                oldpc = x.pc;
                x.pc = joinLabel(x.pc, getLabel(s));
            }
          ...
          case RETURN:
            ...
            x.result = n.value ? getRawValue(execute(n.value, x)) : undefined;
            throw RETURN;
          ...
          case WITH:
            ...
            if (x.pc.c === H && x.pc.i === L)
                x.scope = { ht: t, hu: {}, lt: {}, lu: {},
                            base: {}, parent: x.scope };
            else if (x.pc.c === H && x.pc.i === H)
                x.scope = { ht: {}, hu: t, lt: {}, lu: {},
                            base: {}, parent: x.scope };
            else if (x.pc.c === L && x.pc.i === L)
                x.scope = { ht: {}, hu: {}, lt: t, lu: {},
                            base: {}, parent: x.scope };
            else if (x.pc.c === L && x.pc.i === H)
                x.scope = { ht: {}, hu: {}, lt: {}, lu: t,
                            base: {}, parent: x.scope };
          ...
          case VAR:
          case CONST:
            // Note that this isn't really for declarations but for
            // assignments. The SCRIPT handles declarations by enumerating
            // varDecls.
            c = n.children;
            for (i = 0, j = c.length; i < j; i++) {
                let cell;
                ...
                let val;
                let ref = new Reference(s, t, x.pc, c[i]);
                let initLabel = getLabel(u);
                // Like in ASSIGN, get the label on what's _in_ the ref.
                let m = getLabel(ref, true);
                if (m !== x.pc) {
                    // Unlike in ASSIGN we know the lhs can't be explicitly
                    // labeled (syntactically it can't be).
                    let lhs = isDeclassified(u) ? initLabel : joinLabel(x.pc, m);
                    let rhs = joinLabel(m, getLabel(getRawValue(ref), true));
                    if (valuateLabelC(lhs) > valuateLabelC(rhs) ||
                        valuateLabelI(lhs) > valuateLabelI(rhs))
                        throw new FlowError(lhs + " </= " + rhs);

                    val = initLabel ? new LabeledValue(w, initLabel) : w;
                    val = relabelValue(val, joinLabel(x.pc, m), m);
                } else {
                    val = initLabel ? new LabeledValue(w, initLabel) : w;
                }

                if (n.type === CONST) {
                    definitions.defineProperty(s.base[t], t, val,
          ...
          case ASSIGN:
            ...
            // Slow-path: The label on what's _in_ the ref is not === the pc.
            m = getLabel(w instanceof LabeledValue ? w.r : w, true);
            if (slow = (m && m !== x.pc)) {
                // Declassification can override the implicit label on the
                // value.
                let lhs = isDeclassified(s)
                        ? k
                        : joinLabel(x.pc, getLabel(w, true));
                let rhs = joinLabel(m, u && getLabel(u, true));
                if (valuateLabelC(lhs) > valuateLabelC(rhs) ||
                    valuateLabelI(lhs) > valuateLabelI(rhs))
                    throw new FlowError(lhs + " </= " + rhs);
            }
            ...
            if (slow) {
                ...
                v = relabelValue(v, joinLabel(x.pc, getLabel(w, true)), m);
                putValue(w, v, c[0]);
            } else {
          ...
          case HOOK:
            ...
            r = getRawValue(d, x) ? getRawValue(execute(c[1], x))
          ...
          case OR:
            ...
            t = execute(c[0], x);
            u = execute(c[1], x);
            r = liftL(function(x, y) x || y)(getRawValue(t), getRawValue(u));
            k = joinLabel(getLabel(t), getLabel(u));
            break;
          ...
          case IN:
            ...
            t = execute(c[0], x);
            u = execute(c[1], x);
            r = liftL(function(x, y) x in y)(getRawValue(t), getRawValue(u));
            k = joinLabel(getLabel(t), getLabel(u));
            break;
          ...
          case INSTANCEOF:
            ...
            u = execute(c[1], x);
            w = getRawValue(u);
            if (isObject(w) && typeof w.__hasInstance__ === "function")
                r = w.__hasInstance__(s);
            else
                r = s instanceof w;
            k = joinLabel(getLabel(t), getLabel(u));
            break;
          ...
          case NOT:
            t = execute(n.children[0], x);
            r = liftL(function(x) !x)(getRawValue(t));
            k = getLabel(t);
            break;
          ...
          case LABEL:
            u = execute(n.expression, x);
            r = getRawValue(u);
            // Functions keep labels in themselves, but we need to clone the
            // function to relabel.
            if (typeof r.__call__ === "function") {
                let handler = definitions.makePassthruHandler(r);
                let p = Proxy.createFunction(handler,
                          function() { return r.__call__(this, arguments, x); },
                          function() { return r.__construct__(arguments, x); });
                p.infolabel = joinLabel(n.infolabel, r.infolabel);
                if (valuateLabel(p.infolabel) <= valuateLabel(x.pc))
                    p.infolabel = undefined;
                r = p;
            } else {
                k = joinLabel(n.infolabel, getLabel(u));
                // The <k>^pc check.
                if (valuateLabel(k) <= valuateLabel(x.pc))
                    k = undefined;
            }
            break;
          ...
          case INCREMENT:
          case DECREMENT:
            // Should always be fast-pathed, isn't syntactically possible to
            // have a flow violation.
            t = execute(n.children[0], x);
            u = Number(getRawValue(t));
            k = getLabel(t);
          ...
          case INDEX:
            ...
            t = getRawValue(w);
            u = execute(c[1], x);
            m = String(getRawValue(u));
            s = toObject(t, w, c[0]);
            // Is this a new property?
            r = new Reference(s, m, x.pc, n);
            if (isSlowPath(w, x) || isSlowPath(u, x))
                k = joinLabel(getLabel(w), getLabel(u));
            break;
          ...
          case LIST:
            ...
            c = n.children;
            for (i = 0, j = c.length; i < j; i++) {
                // We don't getRawValue here so that builtin functions like
                // print can examine LabeledValues.
                u = execute(c[i], x);
          ...
          case DECLASSIFY:
            u = execute(n.expression, x);
            m = getLabel(u) || x.pc;
            // Always check integrity of pc and expression to be H.
            if (valuateLabelI(x.pc) > valuateLabelI(new Label(H, H)) ||
                valuateLabelI(m) > valuateLabelI(new Label(H, H)))
                throw new FlowError("cannot declassify when pc or " +
                                    "or expression is not U");
            // Slow path is when the label we're declassifying to isn't the
            // same as the pc.
            if (n.infolabel !== x.pc) {
                // Check that we're note declassifying into a different
                // integrity.
                let k = getLabel(u) || x.pc;
                if (n.infolabel.i !== k.i)
                    throw new FlowError("cannot declassify into " +
                                        "different integrity");
            }
            r = getRawValue(u);
            k = n.infolabel;
          ...
          case CALL:
            ...
            if (slow = (f.infolabel && f.infolabel !== x.pc)) {
                k = f.infolabel;
                // Don't need to save/reset pc here, done in __call__ when
                // making a new ExecutionContext since functions keep labels
                // on themselves.
            }
          ...
          case NEW_WITH_ARGS:
            ...
            if (slow = isSlowPath(w, x)) {
                oldpc = x.pc;
                k = getLabel(w);
                x.pc = joinLabel(x.pc, getLabel(w));
            }

            r = f.__construct__(a, x);

            if (slow) {
                if (valuateLabel(k) <= valuateLabel(oldpc))
                    k = undefined;
                x.pc = oldpc;
            }
            break;
          ...
          case OBJECT_INIT:
            ...
            for (i = 0, j = c.length; i < j; i++) {
                ...
                if (t.type === PROPERTY_INIT) {
                    ...
                    let name = c2[0].value;
                    u = execute(c2[1], x);
                    s[name] = getRawValue(u);
                    r.base[name] = s;
                } else {
                    f = newFunction(t, x);
                    u = (t.type === GETTER) ? '__defineGetter__'
                                            : '__defineSetter__';
                    s[u](t.name, thunk(f, x));
                    r.base[t.name] = s;
                }
            }
            break;
          ...
          case IDENTIFIER:
            ...
            r = new Reference(s || globalScope, n.value, x.pc, n);
            break;
        ...
        return k ? new LabeledValue(r, k, declassified) : r;
    }

    function Activation(f, a) {
        for (var i = 0, j = f.params.length; i < j; i++)
            definitions.defineProperty(this, f.params[i],
                                       getRawValue(a[i]), true);
        definitions.defineProperty(this, "arguments", a, true);
    }

    // Null Activation.prototype's proto slot so that Object.prototype.* does
    // not pollute the scope of heavyweight functions. Also delete its
    // 'constructor' property so that it doesn't pollute function scopes.
    ...
    function newFunction(n, x) {
        ...
        // Functions keep labels within themselves.
        p.infolabel = n.infolabel;
        return p;
    }
    ...
    var FOp = FunctionObject.prototype = {
        ...
        __call__: function (t, a, x) {
            var m = this.infolabel;
            // Is this a declared function or a lambda?
            if (this.node.name) {
                let k = chooseLabel(this.scope.base[this.node.name],
                                    this.scope, x.pc);
                m = joinLabel(m, k);
            } else {
                m = joinLabel(m, x.pc);
            ...
            var x2 = new ExecutionContext(FUNCTION_CODE, joinLabel(x.pc, m));
            x2.thisObject = t || global;
            x2.caller = x;
            x2.callee = this;
            definitions.defineProperty(a, "callee", this, false, false, true);
            var f = this.node;
            var activation = new Activation(f, a);
            var base = mkBase(activation);
            if (x.pc.c === H && x.pc.i === T)
                x2.scope = { ht: activation, hu: {}, lt: {}, lu: {},
                             base: base, parent: this.scope };
            else if (x.pc.c === H && x.pc.i === U)
                x2.scope = { ht: {}, hu: activation, lt: {}, lu: {},
                             base: base, parent: this.scope };
            else if (x.pc.c === L && x.pc.i === T)
                x2.scope = { ht: {}, hu: {}, lt: activation, lu: {},
                             base: base, parent: this.scope };
            else if (x.pc.c === L && x.pc.i === U)
                x2.scope = { ht: {}, hu: {}, lt: {}, lu: activation,
                             base: base, parent: this.scope };
            try {
                x2.execute(f.body);
            } catch (e if e === RETURN) {
    ...
    function evaluate(s, f, l) {
        ...
        var x = new ExecutionContext(GLOBAL_CODE, new Label(L, U));
