    ...
    function execute(n, x) {
        ...
        switch (n.type) {
          ...
          case CONST:
            ...
            for (i = 0, j = c.length; i < j; i++) {
                ...
                if (m !== x.pc) {
                    ...
                    let lhs = joinLabel(x.pc, m);
                    let rhs = joinLabel(m, getLabel(getRawValue(ref)));
                    if (valuateLabel(lhs) > valuateLabel(rhs))
          ...
          case ASSIGN:
            ...
            if (slow = (m !== x.pc)) {
                let lhs = joinLabel(x.pc, getLabel(w));
          ...
          case DOT:
            ...
            if (!s.base[u])
                s.base[u] = x.pc === L ? s.lo : s.hi;
            r = new Reference(s.base[u], u, s.base[u] === s.lo ? L : H, n);
            if (isSlowPath(w, x))
                k = getLabel(w);
            break;
          ...
          case INDEX:
            ...
            s = toObject(t, w, c[0]);
            if (!s.base[m])
                s.base[m] = x.pc === L ? s.lo : s. hi;
            r = new Reference(s.base[m], m, s.base[m] === s.lo ? L : H, n);
