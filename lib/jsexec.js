...
Narcissus.interpreter = (function() {
    ...
    function Reference(base, propertyName, storeLabel, node) {
        this.base = base;
        ...
        switch (n.type) {
          ...
          case CONST:
            ...
            for (i = 0, j = c.length; i < j; i++) {
                ...
                let val;
                let ref = new Reference(s.base[t], t,
                                        chooseLabel(s.base[t], s), c[i]);
                ...
                if (m !== x.pc) {
                    ...
                    let rhs = joinLabel(m, getLabel(getRawValue(ref)));
          ...
          case ASSIGN:
            ...
            m = getLabel(w instanceof LabeledValue ? w.r : w);
            if (slow = (m !== x.pc)) {
                // Declassification can override the implicit label on the
                // value.
                let lhs = isDeclassified(s) ? k : joinLabel(x.pc, getLabel(w));
                let rhs = joinLabel(m, getLabel(u));
                if (valuateLabelC(lhs) > valuateLabelC(rhs) ||
                    valuateLabelI(lhs) > valuateLabelI(rhs))
                    throw new FlowError(lhs + " </= " + rhs);
            }
            ...
            if (slow) {
                ...
                v = relabelValue(v, joinLabel(x.pc, getLabel(w)), m);
                putValue(w, v, c[0]);
            } else {
          ...
          case DOT:
            ...
            if (!s.base[u])
                s.base[u] = chooseBase(x.pc, s);
            r = new Reference(s.base[u], u, chooseLabel(s.base[u], s), n);
            if (isSlowPath(w, x))
                k = getLabel(w);
            break;
          ...
          case INDEX:
            ...
            s = toObject(t, w, c[0]);
            if (!s.base[m])
                s.base[m] = chooseBase(x.pc, s);
            r = new Reference(s.base[m], m, chooseLabel(s.base[m], s), n);
          ...
          case IDENTIFIER:
            ...
            r = new Reference(s && s.base[n.value], n.value,
                              s && chooseLabel(s.base[n.value], s), n);
            break;
    ...
    var FOp = FunctionObject.prototype = {
        ...
        __call__: function (t, a, x) {
            ...
            if (this.node.name) {
                let k = chooseLabel(this.scope.base[this.node.name], this.scope);
                m = joinLabel(m, k);
            } else {
                m = joinLabel(m, x.pc);
            }
