        ...
        switch (n.type) {
          ...
          case WHILE:
            ...
            while (!n.condition || getRawValue(d)) {
                ...
                // Resets the pc after every loop.
                if (slow)
                    x.pc = oldpc;
          ...
          case CONST:
            ...
            for (i = 0, j = c.length; i < j; i++) {
                ...
                let val;
                let ref = new Reference(s.base[t], t, c[i]);
                let initLabel = getLabel(u, x.scope);
                // Like in ASSIGN, get the label on what's _in_ the ref.
                let m = getLabel(ref, x.scope);
                if (m !== x.pc) {
                    ...
                    if (valuateLabel(lhs) > valuateLabel(rhs))
                        throw new Error("flow violation: " +
                                        lhs + " </= " + rhs);
          ...
          case ASSIGN:
            ...
            if (slow = (m !== x.pc)) {
                let lhs = joinLabel(x.pc, getLabel(w, x.scope));
                let rhs = joinLabel(m, getLabel(u, x.scope));
                if (valuateLabel(lhs) > valuateLabel(rhs)) {
          ...
          case OR:
            ...
            k = joinLabel(getLabel(t, x.scope), getLabel(u, x.scope));
            break;
